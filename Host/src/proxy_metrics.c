...
static void push_event_record(char code, proxy_device_t device_type, uint16_t vid, uint16_t pid)
{
...
void metrics_record_event(detection_event_t event)
{
    ...
    {
        case EVENT_ENUM_OK:          code = 'O'; break;
        case EVENT_ENUM_ERROR:       code = 'R'; break;
        case EVENT_BOT_DETECTED:     code = 'B'; break;
        case EVENT_STRIKE:           code = 'S'; break;
        case EVENT_DEVICE_SELECTED:  code = 'D'; break;
        case EVENT_DEVICE_UNMOUNTED: code = 'U'; break;
        default:                     code = 'N'; break;
    }
...
{
    return (uint16_t)keystroke_total;
}

uint16_t metrics_get_event_count(void)
{
    return (uint16_t)event_total;
}
...
void metrics_try_export_cdc(void)
{
    ...
    if (event_pending > 0)
    {
        ...
        uint16_t ev_start = (event_head + EVENT_BUFFER_SIZE - event_pending) % EVENT_BUFFER_SIZE;
        ...
        int ev_len = snprintf(ev_buf, sizeof(ev_buf), "E,%lu,%c,%u,%04x,%04x\r\n",
                              ...
                              ev->code,
                              (unsigned int)ev->device_type,
                              (unsigned int)ev->vid,
                              (unsigned int)ev->pid);

        if (ev_len <= 0 || ev_len >= (int)sizeof(ev_buf)) return;
        if (!cdc_write_record(ev_buf, (uint32_t)ev_len)) return;
    ...
    uint16_t start = (forwarding_head + FORWARDING_BUFFER_SIZE - key_pending) % FORWARDING_BUFFER_SIZE;
